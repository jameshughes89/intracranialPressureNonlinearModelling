...
for tme in times:
	bexp = importlib.import_module('allExpressions_' + tme, package=None)
	funcs = bexp.getFuncs()
	...
	np.savetxt('6-abEerrOfAllModels_' + tme + '.csv', matrixABE, delimiter=",")
	np.savetxt('6-abEerrOfAllModels-NoNaN_' + tme + '.csv', matrixABE[np.logical_not(np.isnan(matrixABE))], delimiter=",")
