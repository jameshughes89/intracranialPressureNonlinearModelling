...
for tme in times:
	bexp = importlib.import_module('bestExpressions_' + tme + '_lessFeatures', package=None)
	funcs = bexp.getFuncs()
	matrixMSE = []
	matrixABE = []
	matrixCorr = []
	...
	np.savetxt('3-msEmat_' + tme + '_lessFeatures.csv', matrixMSE, delimiter=",")
	np.savetxt('3-abEmat_' + tme + '_lessFeatures.csv', matrixABE, delimiter=",")
	np.savetxt('3-corrmat_' + tme + '_lessFeatures.csv', matrixCorr, delimiter=",")
