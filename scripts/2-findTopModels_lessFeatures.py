...
for tme in times:
	...
	for sub in subjects:
		...
		iFile = csv.reader(open('../jGPv9-ICP/src/outs/' + str(sub) + '-' + tme + '_lessFeatures/stats.csv', 'r'))
		...
		iFile = open('../jGPv9-ICP/src/outs/' + str(sub) + '-' + tme + '_lessFeatures/' + str(allBestInd[-1]) + '_line.txt','r')
	...
	oFile = open('bestExpressions_' + tme + '_lessFeatures.py','w')	
	oFile.write("from math import *\n\n")
	fs='funcs = ['
	count = 0
	for l in bestExpressions:
		oFile.write('def func_' + str(count) + '(v0,v1,v2,v3): return ' + l + '\n')		#CHANGE HERE (in function name)
