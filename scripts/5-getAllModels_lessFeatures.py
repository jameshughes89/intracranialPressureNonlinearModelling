...
for tme in times:
	...
	oFile = open('allExpressions_' + tme + '_lessFeatures.py','w')	
	...
	for sub in subjects:
		...
		print sub, tme
		for i in range(50):
			iFile = open('../jGPv9-ICP/src/outs/' + str(sub) + '-' + tme + '_lessFeatures/' + str(i) + '_line.txt','r')

			oFile.write('def func_' + str(sub) + '_' + str(i) + '(v0,v1,v2,v3): return ' + iFile.next() + '\n')
			iFile.close()
