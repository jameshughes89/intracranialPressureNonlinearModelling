...
for tme in times:
	bexp = importlib.import_module('bestLinearExpressions_' + tme + '_lessFeatures', package=None)
	funcs = bexp.getFuncs()
	matrixMSE = []
	matrixABE = []
	matrixCorr = []
	...
	np.savetxt('3-msEmat_Linear_' + tme + '_lessFeatures.csv', matrixMSE, delimiter=",")
	np.savetxt('3-abEmat_Linear_' + tme + '_lessFeatures.csv', matrixABE, delimiter=",")
	np.savetxt('3-corrmat_Linear_' + tme + '_lessFeatures.csv', matrixCorr, delimiter=",")
