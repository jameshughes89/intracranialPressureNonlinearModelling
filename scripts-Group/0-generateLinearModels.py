...
for tme in times:
	...
	# go through each groups for the current time
	for count, g in enumerate(groups):
		print g, tme
		data = np.array(list(csv.reader(open('../PNdata/' + g + '-' + str(tme) + 'groupData.csv', 'r')))).astype(np.float)	# load the data (it's ugly, i know)
		...
		if count != len(groups) - 1:					# don't acutally need this; you can have a trailing comma
			fs = fs + ','	
...
# Uncomment this part if you wanna do subset of features. 
'''
