...
for tme in times:
	bexp = importlib.import_module('bestExpressions_' + tme, package=None)
	funcs = bexp.getFuncs()
	matrixMSE = []
	matrixABE = []
	matrixCorr = []
	...
	for sub in subjects:
		...
		#iFile = csv.reader(open('../PNdata/' + g + '-' + 'all' + 'groupData.csv', 'r'))		
	...
	np.savetxt('3-msEmat_' + tme + '.csv', matrixMSE, delimiter=",")
	np.savetxt('3-abEmat_' + tme + '.csv', matrixABE, delimiter=",")
	np.savetxt('3-corrmat_' + tme + '.csv', matrixCorr, delimiter=",")
